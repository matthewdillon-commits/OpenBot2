...
6. Events reach the Intelligence gateway through `CopilotRuntime.runner.run` on the existing mapped thread (tab turns and unattended jobs). The Activity tab does not survive a reload.
...
boundary. Unattended persist is `CopilotRuntime.runner.run` on the existing mapped thread. See [product.md](product.md).
