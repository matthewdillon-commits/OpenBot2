...
creates a cron job or an inbound webhook (the primitive a later email trigger will fire). Creating
and firing go through the gateway (`intent == "schedule"`). The brief is posted to a hidden task
channel and the coworker is woken afterwards; the HTTP request is not held open for the run. Due
times live in Postgres so a restart still sees work that is due.
