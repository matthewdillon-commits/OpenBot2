...
- **[docs/product.md](docs/product.md) is the LimitlessAI contract.** Part A is the GTM: not a bot builder; one intelligence layer above specialized agents; OBSERVE → UNDERSTAND → PRIORITIZE → ACT → MEASURE → IMPROVE; governed self-improving (see, test, approve, learn); swappable models; CopilotKit is the conversation layer, not the product. Part B is what a deployment does today (turns start in the app or from Send-and-go; computer tools execute on the server; persist onto the Intelligence thread fails closed because CopilotKitIntelligence has no message-write method; orgs are query-scoped, not RLS). Part C is what is not built: cron/webhook/inbound email, self-serve SaaS, the self-improving loop.
