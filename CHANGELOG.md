...
- **Bots can send and read mail.** An administrator stores a write-only SMTP and/or IMAP credential under Admin → Credentials. A deployment with SMTP offers every Bot `send_email`; IMAP offers `read_email` (list recent inbox, or read one message by id). The call is judged by the same policy the other first-party tools are (`tool.name == "send_email"` / `read_email`, or `intent == "email"`) and written on the trail as `email.sent` / `email.send_refused` or `email.read` / `email.read_refused` — destination, subject and the decision, never the body and never the password. Absent credential, the tool is not offered. A later track can start an agent when mail arrives; this one is the governed mailbox, not inbound automation.
