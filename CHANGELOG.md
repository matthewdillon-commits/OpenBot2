...
- **LimitlessAI stands up campaign, research, and sales workers for the job.** The owner says the job in Composer; they do not pick Sales / Website / Marketing from nav. The orchestrator calls `start_specialist` with `kind=campaign` | `kind=research` | `kind=sales`. That worker joins the same goal’s room, enqueues via `enqueueUnattendedJob` (the existing worker still claims with `FOR UPDATE SKIP LOCKED` and calls `startUnattendedRun`), and reports last action on that goal. Always-on sales also writes a standing hourly cron on the goal so the loop wakes without the owner typing again; mailbox and webhook still enqueue the same job when mapped. Packaged workers are not seed channels and not owner-nav items. Existing tenant orgs receive new packaged workers on API boot (copy from the local package org). Mail send is delivered when outbound mail is configured, otherwise logged. Research uses web search and the computer when this deployment has them. Inbound email is still the signed POST — this does not open a live inbox. No PR #11 / #27 / #4–#10.
...
- **Part B: campaign / research / sales workers under one orchestrator.** Citations: `server/src/jobs/worker-kinds.ts`, `server/src/jobs/specialist.ts`, `examples/fintech/agents.yaml`, `server/src/orgs/provision.ts`. Does not claim live mailbox receive or guaranteed SMTP delivery.
