...
- **Part B: cron, webhook, and inbound email enqueue the Phase 1 job.** Citations: `server/src/jobs/enqueue.ts`, `server/src/jobs/triggers.ts`, `server/src/jobs/inbound.ts`. Part C stays honest: persist onto the Intelligence thread still fails closed; there is no Goals home and no Phase 5 approval cards.
...
- **[docs/product.md](docs/product.md) is the LimitlessAI contract.** Part A is the GTM: not a bot builder; one intelligence layer above specialized agents; OBSERVE → UNDERSTAND → PRIORITIZE → ACT → MEASURE → IMPROVE; governed self-improving (see, test, approve, learn); swappable models; CopilotKit is the conversation layer, not the product. Part B is what a deployment does today (turns start in the open app, from Send-and-go, or from cron / webhook / inbound email; computer tools execute on the server; persist onto the Intelligence thread fails closed because CopilotKitIntelligence has no message-write method; orgs are query-scoped, not RLS). Part C is what is not built: a durable unattended transcript, self-serve SaaS, the Goals home and self-improving loop.
