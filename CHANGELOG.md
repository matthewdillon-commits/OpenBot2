...
- **Part B: Composer + Goals, See the work, specialists on demand.** Citations: `app/src/routes/_authed/_app/index.tsx`, `app/src/lib/nav/owner-nav.ts`, `server/src/jobs/specialist.ts`, `server/src/jobs/room-routes.ts`, `app/src/components/channels/see-the-work.tsx`. Part C shrinks: the two doors exist; persist onto the Intelligence thread still fails closed; no Phase 5 approval cards; no Stripe/RLS.
