...
- **Unattended jobs and composer “Say hello” now produce a real Intelligence reply.** After PR #34 minted the mapped thread, both unattended jobs still failed in ~1s with `undefined is not an object (evaluating 'this.#request')`: `withThreadLock` extracted `ɵacquireThreadLock` and called it unbound, so CopilotKitIntelligence’s private `#request` fetch had no `this`. Lock methods are now invoked on the client. Persist is still `getThread` / `getThreadMessages` (prompt + assistant result on that thread) — no local `messages[]`. A first-contact Not Found no longer locks the composer on an unseeded new goal. Intelligence / tab / unattended BuiltInAgent runs send `BOT_MODEL` or `OPENAI_MODEL` (then the tenant package), never a leftover `gpt-4.1` when those say `grok-4.6`. Claim-before-bootstrap, one-connection `UPDATE … RETURNING`, no drizzle transaction, and `runWithRequestRls` / `claim({ bypass: true })` stay.
