...
- **The sign-in screen is LimitlessAI-2's login.** Same grain, logo, Welcome Back, unlabeled rounded fields, navy Sign In, Forgot password, and Continue with Google. Google lights up when `GOOGLE_OAUTH_CLIENT_ID` and `GOOGLE_OAUTH_CLIENT_SECRET` are set; the same address links to an existing email/password user.
