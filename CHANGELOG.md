...
- **Part B: unattended persist is `CopilotRuntime.runner.run`.** Citation: `server/src/jobs/runtime-run.ts`. Part C no longer lists a durable unattended transcript as missing. SaaS leftovers and Stage 4 compounding (no experimentation platform, no Measure nav) remain.
- **Part B: measure and improve on the same goal.** Citations: `server/src/loop/wait.ts`, `server/src/loop/routes.ts`, `server/src/loop/guidance.ts`, `server/src/db/schema/core.ts` `channels.loop`, `app/src/components/channels/goal-loop-card.tsx`. Part C shrinks: approval cards, outcome, and keep / revise / revert exist on the goal.
- **Part B: Composer + Goals, See the work, specialists on demand.** Citations: `app/src/routes/_authed/_app/index.tsx`, `app/src/lib/nav/owner-nav.ts`, `server/src/jobs/specialist.ts`, `server/src/jobs/room-routes.ts`, `app/src/components/channels/see-the-work.tsx`. The two doors exist.
