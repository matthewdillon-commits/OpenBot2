...
- **Goal search no longer claims there are no matches when the match is off the first page.** The sidebar used to filter the 50 channels already loaded, so an older goal vanished under search or All / Active / Completed even though it was still in history. Search and status now go to the server (name, last message, last action; case and surrounding whitespace ignored). “No goals yet” and “No search matches” are different sentences, and the empty copy waits until exiting rows have left. Show more loads the next page.

- **E2B computer template start now binds :4100 during the operator build.** `bun server/e2b/build.ts` first failed because start used `/root/.bun/bin/bun` (exit 126; E2B start is not root). Bun is copied to `/usr/local/bin/bun` (mode 755) and start is `bun src/index.ts`. E2B then runs that start for `waitForPort(4100)`; agent-computer exits 1 without a token. The image has a non-secret placeholder `COMPUTER_TOKEN=template-build` so the healthcheck can bind. Runtime sandbox create still overwrites it with the deployment token. Rebuild the `openbot-agent-computer` template after pulling this.
...
- **Send-and-go follow-ups no longer die on the first thread lock 409.** After an in-tab turn, Intelligence can still hold the mapped thread lock: CopilotKit’s `handleIntelligenceRun` acquires it, and on a successful runner complete it only clears the heartbeat — it does not `ɵcleanupThreadLock`. The grey send-and-go control then POSTed `/api/threads/:id/lock`, got 409 `THREAD_LOCK_FAILED` (`retryable:true`, “Thread is already locked by another runner.”), and the goal kept the user bubble plus “This coworker will continue after you leave.” The tab runner now releases the lock when that run completes — by wrapping `run` / `runWithStartupBoundary` on the existing runner. Assigning `CopilotRuntime.runner` throws (it is a getter) and took the API down before `/api/capabilities` answered. Unattended lock/run retries the 409 with backoff on the same thread. Exhausted retries mark the job failed and the goal shows the error (not a stuck away notice). Primary (black) Send is still in-tab `runAgent` and is not rewritten here.
- **See the work no longer crashes the goal page.** Opening the operator room (or `?work=true`) used to throw `CopilotKitCore.registerProxiedAgent: agentId "channel:…" is already registered` because the owner thread had already registered that id. See the work now shares that instance — it does not register a second CopilotKit agent. Closing the room leaves the owner thread usable. Typical owners still do not see Agents in the nav; Skills and Plugins stay. No second runner. No PR #11 / #27 / #4–#10.
- **Webhook create copy names the secret header.** The one-time “Copy this now” modal now says to send the secret as `Authorization: Bearer` or `x-openbot-trigger-secret`. Other header names and `?secret=` are still 401. The secret is not logged.

- **A typical owner no longer sees leftover OpenBot chrome.** Agents is out of the owner rail; leftover coworker names (General Assistant, Knowledge, Risk Analyst) are not first-class in the owner thread. Skills and Plugins stay. CRM stays the org book. Deployment-wide `/admin` is platform superadmin / `INITIAL_ADMIN_EMAILS`, not an org owner (`/api/admin/status` is 403). The owner thread does not render raw tool traces or Blocked-skill dumps — those stay behind See the work. A second org refused on the shared computer is told they cannot use it yet and to ask their operator; the copy does not name `COMPUTER_SUPERVISOR_URL`. Approval Before/After is human-readable when the stored value was JSON. Home is still Composer + Goals. No PR #11 / #27.
- **Unattended “Say hello” now joins the Intelligence runner after the thread exists.** After PR #36 a live walk (fresh signup `os2-hello36-1787709600`) minted a real thread (`GET /api/threads/:id` known:true, platform `getThread` — not the empty-wrapper lie). `job_b792d2fd` and `job_ab5761a4` then failed in ~16s with `Intelligence runner did not join the thread in time` — no `gpt-4.1`, no `grok-4.6`, no `#request`, no `THREAD_NOT_ON_PLATFORM`. `IntelligenceAgentRunner.runWithStartupBoundary` is a cold Observable: Phoenix `ingestion:${runId}` join starts only on `events.subscribe`. The tab (`handleIntelligenceRun`) subscribes first, then awaits `startup`. Unattended awaited `startup` first, so the socket never connected. Subscribe first, then wait for join, then wait for the run. Persist is still `getThread` / `getThreadMessages` (prompt + assistant result) — no local `messages[]`. Mint fail-closed, lock `.call(intelligence)`, `BOT_MODEL` / `OPENAI_MODEL`, home send, and claim/ALS from #31–#36 stay.
- **Unattended “Say hello” no longer hangs `running` on a thread Intelligence never joined.** After PR #35 the `#request` throw was gone, but a live walk (fresh signup `os2-hello-1787707988`) left `job_2c536729` running 120s+ with `resultText` still the prompt echo and `job_cbb97556` queued because the single worker was stuck in `runner.run`. Worker logs showed get-before-create `THREAD_NOT_FOUND` 404s, then silence — no model line. `openIntelligenceThread` now requires a `getThread` the platform can read (explicit `POST /api/threads` via `createThread` if getOrCreate only returned the mapped id). A mint that still 404s fails the job in seconds. Runner startup that never joins times out in 15s instead of blocking the claim loop. Persist is still `getThread` / `getThreadMessages` (prompt + assistant result) — no local `messages[]`. `BOT_MODEL` / `OPENAI_MODEL`, lock `.call(intelligence)`, home send, and claim/ALS from #31–#35 stay.
- **Unattended jobs and composer “Say hello” now produce a real Intelligence reply.** After PR #34 minted the mapped thread, a live walk on os2 (fresh account `os2-ui-1787706123`, org “UI Persist Walk 1787706123”) still failed both doors: Enter created goal “Say hello” (Active, `/channel/channel_1345411b-…`), the user bubble rendered, the input stayed usable, and there was no red Not Found — but no assistant reply after ~95s. Railway in that window reached `https://api.x.ai/v1/chat/completions` as `gpt-4.1` and 404’d. Unattended jobs still died in ~1s with `undefined is not an object (evaluating 'this.#request')` after `startedAt`, thread `known:true`. Pinning `OPENAI_MODEL=grok-4.6` and redeploy `4d77bb60` (SUCCESS, same `6e86fbe`) did not unblock that: `job_2f06f1bd` still failed in ~1s with `#request` after `startedAt`, thread known false→true. Worker logs around the claim still showed Intelligence `THREAD_NOT_FOUND` 404s (get-before-create inside `getOrCreateThread`). No `gpt-4.1` or `grok-4.6` lines — the job dies before an LLM call. The model pin does not unblock unattended; the required fix is binding `#request` and a `getOrCreateThread` that actually creates on Intelligence. `withThreadLock` extracted `ɵacquireThreadLock` and called it unbound, so CopilotKitIntelligence’s private `#request` fetch had no `this`. Lock methods are now invoked on the client. Persist is still `getThread` / `getThreadMessages` (prompt + assistant result on that thread) — no local `messages[]`. A first-contact Not Found no longer locks the composer on an unseeded new goal. Intelligence / tab / unattended BuiltInAgent runs send `BOT_MODEL` or `OPENAI_MODEL` (then the tenant package), never a leftover `gpt-4.1` when those say `grok-4.6`. The home send arrow now uses that same submit path as Enter; it used to leave the text in the box and fire no request. Claim-before-bootstrap, one-connection `UPDATE … RETURNING`, no drizzle transaction, and `runWithRequestRls` / `claim({ bypass: true })` stay.
- **A job queued after the worker had already claimed one stayed queued.** `processJob` used `AsyncLocalStorage.enterWith` for the job's org. That store is process-wide, so the next `claim()` UPDATE ran as `openbot_rls` scoped to the first org and a queued row in another org never got `started_at`. The first boot claim still worked because ALS was unset. `processJob` now uses `ALS.run` for one job only, and `claim()` bypasses RLS so every org's queued rows stay visible.
- **Unattended jobs stayed queued after the worker logged `worker-boot`.** Claiming waited on loading CopilotKit/MCP/Intelligence (a dynamic `import("./bootstrap")` started before the first `FOR UPDATE`), and `worker-start` / claim lines were JSON that Railway dropped. The worker now prints plain `worker-start` before anything that can hang, claims on a single-connection pool without a drizzle transaction, and only then loads the coworker graph. Persist onto the Intelligence thread can still fail closed after `startedAt`.
- **Unattended jobs stayed queued after s6 started the worker.** The process logged nothing and never set `startedAt` because `worker-start` waited on a static import of the full coworker graph (CopilotKit, MCP, computer, Intelligence). The worker now logs `worker-start`, claims with `FOR UPDATE SKIP LOCKED`, and only then loads that graph. Persist onto the Intelligence thread can still fail closed after `startedAt`.
- **Unattended jobs stayed queued because s6 never started the worker.** The worker service directory and eventsource preload were in the image, but `user/contents.d` listed only migrate, api, postgres, and the computer. s6 never launched the process, so a queued row never got `startedAt`. The worker is now in that boot bundle, still claims with `FOR UPDATE SKIP LOCKED`, and still preloads eventsource.
...
- **A typical owner can put standing wakes on a goal.** Open a goal that already has an Intelligence thread, add a schedule (standing prompt + interval), copy a webhook URL and secret once, or map a mailbox. Disable or remove from the same place. Cron, webhook, and inbound email still enqueue the same `jobs` row through `enqueueUnattendedJob`; the existing worker claim loop ticks due crons (`tickDueCrons` / `FOR UPDATE SKIP LOCKED`). Inbound email is the signed `POST /api/inbound/email` (Mailgun/SendGrid-style) — this does not open a live inbox. No new nav. No second runner. No PR #11 / #27.

- **The first composer send and the first unattended job open a real Intelligence thread.** A new goal still stores one mapped `threadId`. CopilotKit connect used to 404 (`THREAD_NOT_FOUND` / red "Not Found") because Intelligence had never seen that id, and unattended jobs refused the same way. Both doors now go through CopilotRuntime the way a tab turn does: `getOrCreateThread` on the mapped id under the `org:user` key, then the same `ɵacquireThreadLock` + `runner.run` write that `handleIntelligenceRun` uses. A second job attaches to that same thread. Persist is confirmed by `getThread` / `getThreadMessages` (prompt + result on that thread). No local `messages[]` chat store. Fail-closed `THREAD_NOT_FOUND` is gone. Claim-before-graph-load and the ALS claim bypass stay.
...
- **Measure and improve on the same goal.** A high-risk action (CRM create/update/send, computer write/run, MCP write) that the gateway has permitted waits as an approval card on that goal — rationale, expected impact, before/after, rollback — instead of silently acting. Low-risk reads still auto-run. The owner keep / revise / revert on the goal thread (not a new nav). Outcome (worked / didn't / unknown) is stored on the existing channel (`channels.loop`). The next orchestrator turn is told that decision. Home is still Composer to LimitlessAI + Goals (Active | Needs you | Done, last action, time); loop-stage chrome is allowed on the goal row. Rooms stay behind See the work. Persist onto the Intelligence thread still fails closed (`getThread` only). No PR #11 stack.

- **One orchestrator, specialists on demand.** After login a typical owner sees Composer to LimitlessAI and Goals (the existing channels: name, Active | Needs you | Done, last action, time) — not a specialist roster, and not Sales / Website / Marketing / Customer / Ops. Opening a goal talks to the orchestrator. The orchestrator starts a finite specialist (`start_specialist`: a skill/playbook or a coworker, including one with a computer). That enqueue is `enqueueUnattendedJob`; the worker still claims with `FOR UPDATE SKIP LOCKED` and calls `startUnattendedRun`. Specialists share the organization’s CRM. “See the work” (deployment admin, or org owner/admin) opens the A2A room for this goal; Cmd-K Rooms is the same door for power users. A typical owner does not see rooms unless they opened See the work. First-run does not tour the roster. Leftover package names (General Assistant, Knowledge, Risk Analyst) may still exist; they are not the first screen. No PR #11 rooms/sub-agent stack.

- **Cron, webhook, and inbound email enqueue the same unattended job.** A durable `job_triggers` row (org-scoped) names the actor, goal/channel, Intelligence thread, coworker, and standing prompt. When a cron is due, a signed webhook POST arrives, or a message hits a mapped mailbox, the API inserts a `jobs` row through `enqueueUnattendedJob` — the same insert Send-and-go uses. The worker still claims with `FOR UPDATE SKIP LOCKED` and calls `startUnattendedRun`. No cookie Request: the actor is the person stored on the standing row. A missing mapping or thread is a refuse; the trigger does not mint a thread. Cron, webhook, and email jobs may use server-side computer tools and pause as `needs_you`. Persist onto the Intelligence thread still fails closed on this tree (`getThread` only), so a finished job is honestly failed unless a later persist write exists. There is no second runner and no in-process Map.
...
- **Send-and-go continues a channel after the tab closes.** The composer’s Send-and-go control queues a `jobs` row. The worker claims it with `FOR UPDATE SKIP LOCKED` and runs the same built-in or remote AG-UI coworker, with CRM, web search, knowledge, granted MCP, and computer tools when the gateway is configured and the browser is on. The existing Intelligence thread is required — a missing mapping or thread is a refuse. Persist must write that same thread using `CopilotKitIntelligence.getThread` (the read this tree already uses). That client has no message-write method, so persist fails closed and the job is failed, never succeeded. The job row is not a second transcript (prompt + skinny resultText / outcome only). The roster `lastMessage` updates when the job finishes. A finished job also stores a skinny goal outcome on the row (Active | Needs you | Done, last_action, last_action_at, who ran, CRM record ids if a write already returned them) — not an approval card. In this tree the goal is the existing channel plus its Intelligence thread. Cron, webhook, and inbound email enqueue this same row.

### Docs

- **Part B: coworkers use the computer for live web.** Citations: `server/src/jobs/tools.ts` `computerToolsOffered`, `server/src/jobs/specialist.ts` `specialistGetsComputer`, `server/src/computer/computer-tools.ts` `COMPUTER_UNAVAILABLE_OWNER`, `examples/fintech/agents.yaml`, `shared/bot-prompt.ts`. Specialists inherit the org computer unless `with_computer` is false. A down computer is Needs you without env names.
- **Part B: standing wakes on the goal.** Citations: `app/src/components/channels/goal-wakes-card.tsx`, `app/src/lib/triggers/`, `server/src/jobs/claim-loop.ts` `tickDueCrons`. Does not claim live SMTP inbox receive.

- **Part B: self-serve SaaS.** Citations: `server/src/db/rls.ts`, `server/drizzle/0018_saas_billing_rls.sql`, `server/src/billing/stripe.ts`, `server/src/orgs/store.ts` seats, `server/src/orgs/invite-mail.ts`, `server/src/orgs/sso.ts`, `server/src/orgs/spend.ts`, `server/src/telemetry.ts`. Part C item 2 shrinks to persist-fail-closed, one tenant package, and shared Chromium until the supervisor. Home is still Composer + Goals.
- **Part B: measure and improve on the same goal.** Citations: `server/src/loop/wait.ts`, `server/src/loop/routes.ts`, `server/src/loop/guidance.ts`, `server/src/db/schema/core.ts` `channels.loop`, `app/src/components/channels/goal-loop-card.tsx`. Part C shrinks: approval cards, outcome, and keep / revise / revert exist on the goal; persist onto the Intelligence thread still fails closed.
- **Part B: Composer + Goals, See the work, specialists on demand.** Citations: `app/src/routes/_authed/_app/index.tsx`, `app/src/lib/nav/owner-nav.ts`, `server/src/jobs/specialist.ts`, `server/src/jobs/room-routes.ts`, `app/src/components/channels/see-the-work.tsx`. The two doors exist; persist onto the Intelligence thread still fails closed.
...
- **[docs/product.md](docs/product.md) is the LimitlessAI contract.** Part A is the GTM: not a bot builder; one intelligence layer above specialized agents; OBSERVE → UNDERSTAND → PRIORITIZE → ACT → MEASURE → IMPROVE; governed self-improving (see, test, approve, learn); swappable models; CopilotKit is the conversation layer, not the product. Part B is what a deployment does today (Composer + Goals; See the work; specialists on demand via `enqueueUnattendedJob`; turns also start from Send-and-go or cron / webhook / inbound email; computer tools execute on the server; persist onto the Intelligence thread fails closed because CopilotKitIntelligence has no message-write method; RLS + Stripe seats + invite email + per-org SSO + spend caps + OTel; high-risk waits as an approval card on the goal; keep / revise / revert and outcome feed the next orchestrator turn). Part C leftovers: a durable unattended transcript, one tenant package, shared Chromium until the supervisor.
