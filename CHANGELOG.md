...
- **This deployment has a CRM, above Skills in the sidebar.** `/crm` is the book of people, companies, opportunities, campaigns, and conversations — not `/admin/people`, which is who may sign in. The People tab is a dense table: checkboxes, initials, emails, created by (System, a person, or a Bot), company with favicon, phones, relative created date, and job title. The other tabs are real list-and-detail, including linking a person to a company. Bots get `crm_search`, `crm_get`, `crm_create`, and `crm_update`. Every call goes through the gateway (`intent == "crm"` or the tool name) and lands on the trail as `crm.record_read`, `crm.record_written`, or `crm.record_refused`. A deny writes nothing. A Bot write records that Bot as created-by.
- **Inbound email starts a coworker.** When a new inbox message arrives, the matching standing email job wakes the named coworker with a brief that includes from, subject, id, and enough of the body to act. An administrator creates the job under Admin → Schedules (kind: inbound email) and stores an IMAP credential under Admin → Credentials. An in-process poller lists the inbox, remembers a durable cursor so a restart does not re-fire old mail, and calls `fireInbound({ trigger: "email", trusted: true })`. The HTTP webhook still requires the secret. Firing is judged by policy (`intent == "schedule"`) and lands as `schedule.fired` / `schedule.fire_refused` — destination, subject and id, never the body and never the password. Pause stops it; a policy deny writes a refusal and does not wake.
- **Bots can send and read mail.** An administrator stores a write-only SMTP and/or IMAP credential under Admin → Credentials. A deployment with SMTP offers every Bot `send_email`; IMAP offers `read_email` (list recent inbox, or read one message by id). The call is judged by the same policy the other first-party tools are (`tool.name == "send_email"` / `read_email`, or `intent == "email"`) and written on the trail as `email.sent` / `email.send_refused` or `email.read` / `email.read_refused` — destination, subject and the decision, never the body and never the password. Absent credential, the tool is not offered.
- **Standing jobs: cron schedules and inbound triggers.** An administrator can create a job that wakes a named coworker with a brief, without a chat turn — Admin → Schedules. Cron is evaluated in `DEPLOYMENT_TIMEZONE` (default UTC) and is weekday-only unless the job says otherwise. A webhook (`POST /api/triggers/:id` with a one-time secret) is the inbound primitive; inbound email fires the same path from the in-process poller. Creating or firing goes through the gateway (`intent == "schedule"`) and lands on the trail as `schedule.created`, `schedule.refused`, `schedule.fired`, or `schedule.fire_refused`. The request records the run and returns; the coworker is woken afterwards. There is no extra worker container: a cheap poller on the API server claims due rows. The due time is in Postgres, so a restart still sees work that is due and re-enqueues runs that were in flight. In-process dispatch has the same cross-replica limit as message wakes. Pause stops claiming; a policy deny creates nothing.
- **A Bot can start a sub-agent for a finite chunk of work.** `spawn_subagent` takes a goal, success criteria, and what to report back, and returns an id immediately — the parent is not held open. The child is a background run of that same coworker, not a new profile, and it does not talk to the person. When it finishes or hits a real blocker it reports, and the parent is woken the way a `message_agent` recipient is. A follow-up names the same id so the work stays on that worker; a second spawn without an id is independent. Spawn and the report go through the gateway (`intent == "spawn"`) and land on the trail as `subagent.started`, `subagent.refused`, or `subagent.reported`. The child has no composer; a person sees that it ran on the audit trail and Activity. The task channel that holds the brief is hidden from the roster. The child is offered this coworker's computer (browser, files, and shell) through the same gateway as the parent, plus MCP, knowledge, and web search — not messaging or another spawn. A Bot has one computer; the parent and any child take turns on it. If the child needs a person (a login, a secret), it reports blocked rather than waiting. A chain of follow-ups without a person stops after eight hops, the same ceiling coworker messaging uses.
- **Bots can message each other, and a room can be named.** A Bot has `message_agent` and `message_channel`: the first opens or reuses a 1:1 with another coworker, the second posts to a room it already belongs to. The send returns immediately; the recipient is woken later and may reply. A real reply is stored and wakes the other members, so a room can talk for a few turns without a person; empty acknowledgements are still refused. A chain without a person stops after eight hops. Every send goes through the gateway and lands on the audit trail as `channel.message_sent` or `channel.message_refused`. A person can name a room when seating two or more coworkers, and add or remove members from channel settings. Sidebar and home list those rooms the same way they list every other channel.
