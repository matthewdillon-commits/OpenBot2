...
- **A follow-up on an existing goal gets a reply instead of going silent.** After the first in-tab turn, Intelligence still held the thread lock: CopilotKit’s `handleIntelligenceRun` acquires it, and on a successful runner complete it only clears the heartbeat — it does not `ɵcleanupThreadLock`. Send-and-go / unattended `startUnattendedRun` then POSTed `/api/threads/:id/lock`, got 409 `THREAD_LOCK_FAILED` (`retryable:true`, “Thread is already locked by another runner.”), and the goal kept the user bubble plus “This coworker will continue after you leave.” Same mapped thread. The tab runner now releases the lock when that run completes. Unattended lock/run retries the 409 with backoff instead of failing the first one. If retries are exhausted the job is marked failed and the goal shows the error (not a stuck away notice). Primary Send is still in-tab `runAgent`; send-and-go is the other control.
