...
- **Unattended jobs and composer “Say hello” now produce a real Intelligence reply.** After PR #34 minted the mapped thread, a live walk on os2 (fresh account `os2-ui-1787706123`, org “UI Persist Walk 1787706123”) still failed both doors: Enter created goal “Say hello” (Active, `/channel/channel_1345411b-…`), the user bubble rendered, the input stayed usable, and there was no red Not Found — but no assistant reply after ~95s. Railway in that window reached `https://api.x.ai/v1/chat/completions` as `gpt-4.1` and 404’d. Unattended jobs still died in ~1s with `undefined is not an object (evaluating 'this.#request')` after `startedAt`, thread `known:true`. Pinning `OPENAI_MODEL=grok-4.6` and redeploy `4d77bb60` (same `6e86fbe`) did not unblock that: `job_2f06f1bd` still failed in ~1s with `#request` before any LLM line. `withThreadLock` extracted `ɵacquireThreadLock` and called it unbound, so CopilotKitIntelligence’s private `#request` fetch had no `this`. Lock methods are now invoked on the client. Persist is still `getThread` / `getThreadMessages` (prompt + assistant result on that thread) — no local `messages[]`. A first-contact Not Found no longer locks the composer on an unseeded new goal. Intelligence / tab / unattended BuiltInAgent runs send `BOT_MODEL` or `OPENAI_MODEL` (then the tenant package), never a leftover `gpt-4.1` when those say `grok-4.6`. The home send arrow now uses that same submit path as Enter; it used to leave the text in the box and fire no request. Claim-before-bootstrap, one-connection `UPDATE … RETURNING`, no drizzle transaction, and `runWithRequestRls` / `claim({ bypass: true })` stay.
